...
import { FiDollarSign, FiTrendingUp, FiArrowUpRight, FiClock, FiPlus, FiCreditCard } from 'react-icons/fi'
...
export default function SellerWallet() {
  ...
  const handleAddCredits = async () => {
    ...
    try {
      ...
      dispatch(updateUserInfo({ credits: response.credits }))
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className="text-5xl font-black tracking-tighter">{user?.credits || 0}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100">
            ...
            <div className="space-y-6">
              <div className="relative group">
                <label className="absolute left-6 -top-3 bg-white px-2 text-[10px] font-black text-blue-600 tracking-widest uppercase z-10">Custom Amount</label>
                <input 
                  type="number"
                  value={amount}
              ...
              <p className="text-center text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                <FiCreditCard className="w-3 h-3" /> Secure direct credit system active
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
             ...
             <div className="space-y-4">
                {[
                  { label: "Commission Ded.", val: "-15", date: "Today" },
                  { label: "Recharge", val: "+500", date: "Yesterday", isPos: true },
                  { label: "Commission Ded.", val: "-12", date: "2 days ago" }
                ].map((item, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
                    ...
                    <span className={`text-xs font-black ${item.isPos ? 'text-emerald-500' : 'text-slate-900'}`}>{item.val}</span>
                  </div>
                ))}
             </div>
          </div>
