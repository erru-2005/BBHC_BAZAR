    ...
    <section className="lg:hidden py-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        {labels.map((item) => {
          ...
          return (
          <div key={entry.label} className="flex flex-col items-center flex-shrink-0">
            <div className="w-16 h-16 rounded-full bg-white shadow flex items-center justify-center overflow-hidden">
              {entry.image ? (
                ...
                <span className="text-lg font-bold text-[#131921]">{entry.label.slice(0, 1)}</span>
              )}
            </div>
            <p className="text-xs text-slate-600 mt-2">{entry.label}</p>
          </div>
        )})}
      </div>
    </section>
