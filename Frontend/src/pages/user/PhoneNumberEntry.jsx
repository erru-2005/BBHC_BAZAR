...
function PhoneNumberEntry() {
  ...
  const prefillEmail = useMemo(() => location.state?.prefillEmail || '', [location.state?.prefillEmail])
  ...
  }, [prefillEmail, isAuthenticated, userType, navigate])
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await sendUserOTP(email.trim())
      // Navigate to OTP verification page with session ID, email, and returnTo
      navigate('/user/verify-otp', {
        state: {
          otpSessionId: response.otp_session_id,
          email: email.trim(),
          emailMasked: response.email_masked,
          returnTo: returnTo
        }
      })
    } catch (err) {
      ...
      <div className="w-full lg:w-1/2 max-w-md lg:max-w-[420px] mx-auto">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-white border border-gray-200 shadow-[0_35px_80px_rgba(15,23,42,0.08)] p-8 sm:p-10">
          ...
          <div className="relative text-center mb-8">
            ...
            <h1 className="text-3xl font-semibold text-gray-900 mb-2">Enter Your Email</h1>
            <p className="text-base text-gray-500">We'll send a verification OTP to your email inbox</p>
            {message && (
